...
export const authDB = {
  ...
  async create(data: any) {
    const id = generateId();
    const authData = { ...data, _id: id, createTime: new Date().toISOString() };
  ...
  async getById(id: string) {
    return await kv.get(`${KEYS.AUTHORIZATIONS}${id}`);
  },
  
  // 根据账号获取
  async getByAccount(account: string) {
    const all = await this.getAll();
    return all.find((a: any) => a.account === account);
  },
  ...
  async update(id: string, data: any) {
    ...
    const updated = { ...existing, ...data, updateTime: new Date().toISOString() };
...
export const adminDB = {
  ...
  async getByUsername(username: string) {
    ...
    for (const key of keys) {
      const admin = await kv.get(key);
      if (admin && (admin as any).username === username) {
        return admin;
      }
    }
    return null;
...
export const logDB = {
  ...
  async create(data: any) {
    const id = generateId();
    const logData = { ...data, _id: id, timestamp: new Date().toISOString() };
